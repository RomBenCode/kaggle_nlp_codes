...
for sentence in train_valid_df["text"]:
    sentiment_dict = sid_obj.polarity_scores(sentence)
    list_vader_train_valid.append([sentiment_dict['neg'], sentiment_dict['neu'], sentiment_dict['pos'], sentiment_dict['compound']])

list_vader_test = []
for sentence in test_df:
    sentiment_dict = sid_obj.polarity_scores(sentence)
    list_vader_test.append([sentiment_dict['neg'], sentiment_dict['neu'], sentiment_dict['pos'], sentiment_dict['compound']])
...
emb = Embedding(output_dim=300, input_dim=len(word_index) + 1, input_length=max_len)(nlp_input)
...
nlp_out = Bidirectional(LSTM(100, dropout=0.5, recurrent_dropout=0.5))(emb)
...
#history = model.fit(xtrain_pad, ytrain_enc, batch_size=512, epochs=50, validation_split=0.8, callbacks=[checkpoint_cb])
history = model.fit([xtrain_pad, np.asarray(list_vader_train_valid)], ytrain_enc, batch_size=512, epochs=100, validation_split=0.8, callbacks=[checkpoint_cb])

predict_test = model.predict([xtest_pad,np.asarray(list_vader_test)])
...
data_frame_predict.to_csv(path_or_buf="submission_test_V2.csv", index=False)
